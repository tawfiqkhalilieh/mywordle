  ...
  var VanillaToast = (function () {
    ...
    VanillaToast.prototype.show = function (text, option, callback) {
      ...
      self.queue.enqueue(function (next) {
        // time setting
        var fadeDuration =
          option.fadeDuration || constants.default.fadeDuration;
        var fadeInterval =
          option.fadeInterval || constants.default.fadeInterval;
        var fadeStep = Math.min(fadeInterval / fadeDuration, 1);
        var duration = option.duration || constants.default.duration;

        // close button setting
        self.element.closeButton.style.display = option.closeButton
          ? "inline"
          : "none";
    ...
    function _setStyle(self, option) {
      self.element.toastBox.className =
        option.className || constants.default.className;
    }
